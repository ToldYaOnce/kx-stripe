...
# Build and Publish Script for @toldyaonce/kx-stripe-runtime
...
echo
print_warning "About to publish @toldyaonce/kx-stripe-runtime@$NEW_VERSION"
...
print_success "📦 @toldyaonce/kx-stripe-runtime@$NEW_VERSION is now available"

# Step 10: Git operations (only if in git repo)
if [ -d "../../.git" ]; then
    print_status "📝 Creating git commit and tag..."
    cd ../..
    git add packages/kx-stripe-runtime/package.json
    git commit -m "chore(kx-stripe-runtime): bump version to $NEW_VERSION" || true
    git tag "kx-stripe-runtime-v$NEW_VERSION"
    
    print_status "⬆️  Pushing to git..."
    git push origin main || git push origin master
    git push origin "kx-stripe-runtime-v$NEW_VERSION"
...
echo "  • Install: npm install @toldyaonce/kx-stripe-runtime@$NEW_VERSION"
